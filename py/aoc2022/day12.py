...
def part1(lines):
    ...
    31
    """
    start, = ((y, x) for y, line in enumerate(lines) for x, c in enumerate(line) if c == 'S')
        ...
        for y1 in range(y0 - 1, y0 + 2):
            ...
            for x1 in range(x0 - 1, x0 + 2) if y0 == y1 else (x0,):
                if x1 not in range(len(lines[y1])):
                    continue
                b = lines[y1][x1]
                if b == 'a' if a == 'S' else a == 'z' if b == 'E' else ord(b) - ord(a) <= 1:
                    yield y1, x1
...
def part2(lines):
    ...
    29
    """
    start, = ((y, x) for y, line in enumerate(lines) for x, c in enumerate(line) if c == 'E')
